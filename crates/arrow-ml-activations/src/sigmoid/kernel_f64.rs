...
/// Fast exp(x) approximation for f64 SIMD using range reduction + polynomial.
#[inline(always)]
fn fast_exp(x: Simd<f64, SIMD_WIDTH>) -> Simd<f64, SIMD_WIDTH> {
    ...
    // Higher-order polynomial for f64 precision
    ...
    // 2^floor_z via bit manipulation: reinterpret (1023 + n) << 52 as f64
