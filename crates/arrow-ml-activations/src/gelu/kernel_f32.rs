...
#[inline(always)]
fn fast_tanh(x: Simd<f32, SIMD_WIDTH>) -> Simd<f32, SIMD_WIDTH> {
    let two = Simd::splat(2.0f32);
...
pub fn gelu(input: &[f32]) -> Vec<f32> {
    ...
    let half = Simd::<f32, SIMD_WIDTH>::splat(0.5);
    let one = Simd::<f32, SIMD_WIDTH>::splat(1.0);
    let sqrt_2_over_pi = Simd::<f32, SIMD_WIDTH>::splat(0.7978845608f32);
    let coeff = Simd::<f32, SIMD_WIDTH>::splat(0.044715f32);
