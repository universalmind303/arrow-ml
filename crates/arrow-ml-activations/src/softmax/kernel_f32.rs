...
pub fn softmax(input: &[f32]) -> Vec<f32> {
    ...
    let mut max_v = Simd::<f32, SIMD_WIDTH>::splat(f32::NEG_INFINITY);
    ...
    let max_splat = Simd::<f32, SIMD_WIDTH>::splat(max_val);
    let mut output = Vec::with_capacity(len);
    let mut sum_v = Simd::<f32, SIMD_WIDTH>::splat(0.0);
    ...
    let inv_sum = Simd::<f32, SIMD_WIDTH>::splat(1.0 / sum);

    for i in 0..chunks {
        let offset = i * SIMD_WIDTH;
        let v = Simd::<f32, SIMD_WIDTH>::from_slice(&output[offset..]);
