...
//! libraries matching `libarrow_kernels_backend_*{.dylib,.so,.dll}`, loads
...
fn dylib_glob_pattern() -> &'static str {
    if cfg!(target_os = "macos") {
        "libarrow_kernels_backend_*.dylib"
    } else if cfg!(target_os = "windows") {
        "arrow_kernels_backend_*.dll"
    } else {
        "libarrow_kernels_backend_*.so"
    }
}
...
impl BackendRegistry {
    ...
    fn discover() -> BackendRegistry {
        ...
        for dir in Self::search_dirs() {
            ...
            for entry in glob.flatten() {
                if let Some(backend) = Backend::load(&entry) {
                    eprintln!(
                        "[arrow-kernels] loaded backend {:?} (priority {}) from {}",
                        backend.name,
                        backend.priority,
                        entry.display()
                    );
        ...
        if backends.is_empty() {
            eprintln!("[arrow-kernels] no GPU backends found, using CPU kernels");
        }
    ...
    fn search_dirs() -> Vec<PathBuf> {
        ...
        // 1. Explicit env var
        if let Ok(dir) = std::env::var("ARROW_KERNELS_BACKEND_DIR") {
